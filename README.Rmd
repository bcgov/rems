...
There are two functions to select all of the active sites in the long-term lake monitoring program. The `lt_lake_sites` function selects the `EMS_ID` of active sites. The `lt_lake_req` function selects the `REQUISITION_ID` of active sites. Using the `lt_lake_sites` will provide all data collected under the active `EMS_ID`, using `lt_lake_req` will filter data collected only by the long-term lakes monitoring group. Both functions can be used with `filter_ems_data` to easily pull all data from active long-term lake monitoring sites. 
...
filtered_2yr_lt_lakes_req <- filter_ems_data(two_year, req_id = lt_lake_red(),
  parameter = c("Aluminum Total", "Cadmium Total",
    "Copper Total", " Zinc Total",
    "Turbidity"))
